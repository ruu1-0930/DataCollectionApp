...
- 不引入压力传感器的物理满量程标定（硬件未给），用每帧自动归一化。
- 不改后端/数据库/上传链路。
...
- **每个点渲染**：
  - 颜色：按归一化值着色。归一化满量程 `pmax = max(当前帧左右 18 个压力值)`；`t = pmax > 0 ? v / pmax : 0`；浅蓝（低）→ 深蓝（高）。`pmax=0`（没踩）时全部浅色。
  - 数字：整数压力值印在点上，字号约 9px + 1px 白色描边保证可读（前掌 2/3/4 点较密）。
